...
using static Assets.Scripts.Util.Achievements;
...
public static class PhysicsHelper
{
	...
	/// <summary>
	/// Get all moles in the atmosphere as an enumeration.
	/// </summary>
	/// <param name="atmosphere">The atmosphere to get moles from.</param>
	/// <param name="matterState">The state of matter filter to apply to the enumeration.</param>
	/// <returns>An enumeration of moles in the atmosphere.</returns>
	...
	/// <summary>
	/// Get all moles in the gas mixture as an enumeration.
	/// </summary>
	/// <param name="gasMixture">The gas mixture to get moles from.</param>
	/// <param name="matterState">The state of matter filter to apply to the enumeration.</param>
	/// <returns>An enumeration of moles in the gas mixture.</returns>
	...
	public static MoleEnergy GetGasEnergy(this ref GasMixture mixture) =>
		new(mixture.Moles(MatterState.Gas).Sum(x => x.Energy.ToDouble()));
	...
	public static HeatCapacity GetGasHeatCapacity(this ref GasMixture mixture) =>
		new(mixture.Moles(MatterState.Gas).Sum(x => x.HeatCapacity.ToDouble()));
	...
	public static double GetGasHeatCapacityRatio(this ref GasMixture mixture)
	{
		...
		return gasses < Chemistry.MINIMUM_QUANTITY_MOLES
			...
			: mixture.Moles(MatterState.Gas).Select(mole => (mole.Quantity / gasses * mole.HeatCapacityRatio()).ToDouble()).Sum();
	}
	...
	public static VolumeLitres GetAdiabaticVolumeForTargetPressure(this ref GasMixture mixture, VolumeLitres initialVolume, PressurekPa targetPressure)
	{
		...
		var result = (initialVolume * (float)Math.Pow((initialPressure / targetPressure).ToDouble(), 1.0 / heatCapacityRatio)) + mixture.VolumeLiquids; // add back the subtracted volume of liquids.
	...
	{
		Debug.Assert(fraction is >= 0.0f and <= 1.0f, "Fraction must be in range [0, 1].");
		var result = gas;
	...
	public static double GetAverageGasMolarMass(this ref GasMixture mixture)
	{
		...
		if (totalMoles < Chemistry.MINIMUM_QUANTITY_MOLES)
			return 0.0;
		return mixture.Moles(MatterState.Gas).Sum(x => x.GetMass()) / totalMoles.ToDouble();
	}
