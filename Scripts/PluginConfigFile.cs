...
namespace Entropy.Scripts
{
    public class PluginConfigFile : ConfigFile
    {
        ...
        private void Init()
        {
            ...
            this.CascadedSEGIEntry = Bind(PatchCategory.SEGI.GetDisplayName(), ConfigEntry.CascadedSEGI.GetDisplayName(), false);
            this.SEGIPresetEntry = Bind(PatchCategory.SEGI.GetDisplayName(), ConfigEntry.SEGIPreset.GetDisplayName(), SEGIManager.SettingPreset.High);
            this.SEGICascadedPresetEntry = Bind(PatchCategory.SEGI.GetDisplayName(), ConfigEntry.CascadedSEGIPreset.GetDisplayName(), SEGIManager.SettingCascadedPreset.Standard);

            this.ConesEntry = BindSEGI(ConfigEntry.Cones, 13, 1, 128);
            this.SecondaryConesEntry = BindSEGI(ConfigEntry.SecondaryCones, 6, 3, 16);
            this.ConeTraceStepsEntry = BindSEGI(ConfigEntry.ConeTraceSteps, 8, 1, 32);
            this.ConeLengthEntry = BindSEGI(ConfigEntry.ConeLength, 1f, 0.1f, 2.0f);
            this.ConeWidthEntry = BindSEGI(ConfigEntry.ConeWidth, 6f, 0.5f, 6.0f);
            this.ConeTraceBiasEntry = BindSEGI(ConfigEntry.ConeTraceBias, 0.63f, 0.0f, 4.0f);

            this.DoReflectionsEntry = BindSEGI(ConfigEntry.DoReflections, true);
            this.ReflectionStepsEntry = BindSEGI(ConfigEntry.ReflectionSteps, 64, 12, 128);
            ...
            this.TemporalBlendWeightEntry = BindSEGI(ConfigEntry.TemporalBlendWeight, 0.15f, 0.01f, 1.0f);
            this.GaussianMipFilterEntry = BindSEGI(ConfigEntry.GaussianMipFilter, false);

            this.NearLightGainEntry = BindSEGI(ConfigEntry.NearLightGain, 1f, 0.0f, 4.0f);
            this.GIGainEntry = BindSEGI(ConfigEntry.GIGain, 1f, 0.0f, 4.0f);
            this.NearOcclusionStrengthEntry = BindSEGI(ConfigEntry.NearOcclusionStrength, 0f, 0.0f, 4.0f);
            this.OcclusionPowerEntry = BindSEGI(ConfigEntry.OcclusionPower, 1f, 0.001f, 4.0f);
            this.OcclusionStrengthEntry = BindSEGI(ConfigEntry.OcclusionStrength, 1f, 0.0f, 4.0f);
            this.FarOcclusionStrengthEntry = BindSEGI(ConfigEntry.FarOcclusionStrength, 1f, 0.1f, 4.0f);
            this.FarthestOcclusionStrengthEntry = BindSEGI(ConfigEntry.FarthestOcclusionStrength, 1f, 0.1f, 4.0f);

            this.InnerOcclusionLayersEntry = BindSEGI(ConfigEntry.InnerOcclusionLayers, 1, 0, 2);
            this.InfiniteBouncesEntry = BindSEGI(ConfigEntry.InfiniteBounces, true);
            this.SecondaryBounceGainEntry = BindSEGI(ConfigEntry.SecondaryBounceGain, 1f, 0.0f, 2.0f);
            this.SecondaryOcclusionStrengthEntry = BindSEGI(ConfigEntry.SecondaryOcclusionStrength, 1f, 0.1f, 4.0f);
        ...
        private ConfigEntry<T> BindSEGI<T>(ConfigEntry entry, T defaultValue)
        {
            ...
            return Bind(PatchCategory.SEGI.GetDisplayName(), entry.GetDisplayName(), defaultValue, description);
        }
