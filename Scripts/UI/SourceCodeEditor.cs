...
using TextEditor = Entropy.Assets.Scripts.UI.TextEditor.TextEditor;
...
namespace Entropy.Assets.Scripts
{
	public class SourceCodeEditor : MonoBehaviour
	{
		private static class IC10LanguageDefinition
		{
			...
			{
				mName = "IC10",
				mKeywords = new HashSet<string>(ChipProcessor.GetInstructions()),
				Identifiers = ChipProcessor.GetEnumerationValues().ToDictionary(x => x, x => new Identifier { }),
