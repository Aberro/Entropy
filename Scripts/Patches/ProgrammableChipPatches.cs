...
namespace Entropy.Assets.Scripts.Patches
{
	...
	public class ProgrammableChip_SetSourceCode_Patch
	{
		...
		{
			var processor = ChipProcessor.Translate(__instance);
	...
	public class ProgrammableChip_Execute_Patch
	{
		...
		public static bool Prefix(ProgrammableChip __instance, int runCount)
		{
			...
			if (processor == null)
			{
				processor = ChipProcessor.Translate(__instance);
				__instance.SetExtension(processor);
			}
#if !PROFILE
			...
			public static void Postfix(ProgrammableChip __instance, int runCount)
			{
				stopwatch.Stop();
