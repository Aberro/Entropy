...
public class Parser
{
	public static LineOfCode[] Parse(string code, out Dictionary<string, double> defines, out Dictionary<string, int> labels, out List<string> aliases, out List<DebugSourceLine> sourceLines)
	{
		var inputStream = new AntlrInputStream(code);
		var lexer = new MIPSLexer(inputStream);
		var tokenStream = new CommonTokenStream(lexer);
		var parser = new MIPSParser(tokenStream);
		sourceLines = new List<DebugSourceLine>();
	...
	{
		internal static LineOfCode[] Parse(MIPSParser parser, Dictionary<string, double> defines, List<string> aliases, Dictionary<string, int> labels, List<DebugSourceLine> sourceLines)
		{
		...
		private readonly List<DebugSourceLine> _sourceLines;
		private List<LineOfCode> _lines = new();
		private int _currentLine = -1;
		private LineParser(Dictionary<string, double> defines, List<string> aliases, Dictionary<string, int> labels, List<DebugSourceLine> sourceLines)
		{
		...
		public override void ExitLine([NotNull] MIPSParser.LineContext context)
		{
			...
			{
				this._sourceLines.Add(new DebugSourceLine(source));
				this._lines.Add(new LineOfCode(this._currentLine, source));
				return;
			}
			LineOfCode line;
			...
			{
				line = new LineOfCode(this._currentLine, source, "define", new Variable
				{
			...
			else
			{
				...
				var formattedArgs = new List<DebugArgument>();
				...
				for(var i = 0; i < operands.Length; i++)
				{
					...
					if ((variables[i].Kind &
						...
						| VariableKind.Alias)) != VariableKind.None)
					{
						sourceFormatted.Insert(operand.Start.Column + operand.GetText().Length, $"{{{formatIndex}}}");
						if (variables[i].Kind == VariableKind.Register)
							formattedArgs.Add(new DebugArgument {  Target = AliasTarget.Register, Index = variables[i].RegisterIndex });
						else if (variables[i].Kind == VariableKind.Alias) formattedArgs.Add(new DebugArgument { Target = AliasTarget.Alias, Index = variables[i].AliasIndex });
					}
				}
				for(var i = operands.Length; i < 6; i++) variables[i] = Variable.None;
				line = new LineOfCode(this._currentLine, sourceFormatted.ToString(), command, variables[0], variables[1], variables[2], variables[3], variables[4], variables[5]);
				this._sourceLines.Add(new DebugSourceLine(source, formattedArgs.ToArray()));
			}
			this._lines.Add(line);
		}

		private Variable ParseOperand(MIPSParser.OperandContext context)
		{
			...
			if(identifier != null)
			{
				...
				var isLabel = this._labels.TryGetValue(name, out var label);
				var isDefine = this._defines.TryGetValue(name, out var define);
				if(aliasIndex >= 0)
					return new Variable
					{
		...
		private LineOfCode ParseAlias(MIPSParser.AliasContext alias, string source)
		{
			...
			return new LineOfCode(this._currentLine, source, "alias", variable);
		}

		private Variable ParseRegister(MIPSParser.RegisterContext context)
		{
		...
		private Variable ParseDevice(MIPSParser.DeviceContext context)
		{
