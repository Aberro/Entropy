...
public static class PhysicsHelper
{
	...
	public static double Sum(ref GasMixture mixture, Func<Mole, double> selector, MatterState matterState = MatterState.All)
	{
	...
	public static MoleEnergy GetGasEnergy(this ref GasMixture mixture) =>
		new(Sum(ref mixture, x => x.Energy.ToDouble(), MatterState.Gas));
	...
	public static HeatCapacity GetGasHeatCapacity(this ref GasMixture mixture) =>
		new(Sum(ref mixture, x => x.HeatCapacity.ToDouble(), MatterState.Gas));
	...
	public static double GetAverageGasMolarMass(this ref GasMixture mixture)
	{
		...
		if (totalMoles < Chemistry.MINIMUM_QUANTITY_MOLES)
			return 0.0;
		return Sum(ref mixture, x => x.GetMass(), MatterState.Gas) / totalMoles.ToDouble();
	}
	...
	public static double GetAverageMolarMass(this ref GasMixture mixture)
	{
		...
		if (totalMoles < Chemistry.MINIMUM_QUANTITY_MOLES)
			return 0.0;
		return Sum(ref mixture, x => x.GetMass()) / totalMoles.ToDouble();
	}
